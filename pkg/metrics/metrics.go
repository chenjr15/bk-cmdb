...
func NewService(conf *Config) *Service {
	...
	// register user total metrics for web server
	if conf.ProcessName == config.WebServer {
		srv.userTotal = prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Name: Namespace + "_user_http_request_total",
				Help: "user http request total.",
			},
			[]string{LabelUser, LabelTenantId},
		)
		registerer.MustRegister(srv.userTotal)
	}
...
func (s *Service) recordMetrics(kt *kit.Kit, before time.Time, protocol, handler, status string) {
	...
	// collect user total metrics for web-server
	if s.service == config.WebServer {
		s.userTotal.With(prometheus.Labels{
			LabelUser:     kt.User,
			LabelTenantId: kt.TenantID,
		}).Inc()
	}
